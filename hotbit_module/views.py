...
async def compare(asks, bids, where, to, symbols, percent):
    ...
    if type(asks) is float:
        for bid in bids:
            if (float(bid[0]) * percent / 100 + float(bid[0])) > asks:
    ...
    if ask_price > 0 and bid_price > 0 and volume > 0:
        # print('/--------------------------')
        # print('/ ' + w_symbol + ' from ' + where + ' to ' + t_symbol + ' ' + to + ' /')
        # print('/ ask ' + str(ask_price) + ' bid ' + str(bid_price) + ' volume ' + str(volume) + ' /')
        # print('--------------------------/')
        return [w_symbol, where, t_symbol, to, ask_price, bid_price, volume]
    else:
        return None
...
async def compare_markets(symbol, percent, proxy):
    ...
    if hotbit_depth['error'] is None:
        try:
            print('idex', proxy[0], idex_ticker['code'])
        except:
            if idex_ticker[0]['ask']:
                idex_ask = float(idex_ticker[0]['ask'])
            else:
                idex_ask = None
            if idex_ticker[0]['bid']:
                idex_bid = float(idex_ticker[0]['bid'])
            else:
                idex_bid = None
            hitbit_asks = hotbit_depth['result']['asks']
            hitbit_bids = hotbit_depth['result']['bids']
            a = await compare(idex_ask, hitbit_bids, 'idex', 'hotbit', symbol, percent)
            if a is not None:
                compares.append(a)
            b = await compare(hitbit_asks, idex_bid, 'hotbit', 'idex', symbol, percent)
            if b is not None:
                compares.append(b)
    return compares
