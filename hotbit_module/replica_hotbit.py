...
from exchange_comparison.market_depth import get_hotbit_depth

counts = len(gv.proxys4) * 10


async def get_depth(token, tsymbol, cnt):
    depth = await get_hotbit_depth(token, cnt)
    direction = None
    if 'ETH' in token:
        direction = 'ETH'
    if 'BTC' in token:
...
def make_replica():
    replica = {}
    tokens = gt(module='hotbit', _all=False).tokens()
    ...
    for i in range(xlen):
        ...
        for res in init_result:
            replica.update(res)
    gv.replica_hotbit = replica
    # print(gv.replica_hitbtc)
    # print(len(gv.replica_hotbit))
    # print('//====================================//')


async def init_replica_hotbit():
    while True:
        tstart = datetime.now()
        await make_replica()
        print('hotbit replica create time: ', str(datetime.now() - tstart))
