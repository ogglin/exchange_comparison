...
class WebsocketLogSet(viewsets.ModelViewSet):
    permission_classes = [HasAPIKey]
    queryset = WebsocketLog.objects.order_by('-id').filter(Q(token__isnull=False) & ~Q(log__icontains='error'))[:40]
...
class ExchangePairSet(viewsets.ModelViewSet):
    ...
    queryset = CustomSql.objects.raw('''
        WITH tp AS (SELECT trusted_pairs.id, trusted_pairs.tsymbol, trusted_pairs.contract FROM trusted_pairs
              WHERE ((trusted_pairs.is_active IS TRUE) AND (trusted_pairs.contract IS NOT NULL))
              ORDER BY trusted_pairs.tsymbol
            ),
    
            idex as (SELECT tp.id, tp.tsymbol, tp.contract, me.exch_direction, me.highest_bid sell, me.lowest_ask buy, 'idex' site FROM module_idex me
            ...
            hotbit as (SELECT tp.id, tp.tsymbol, tp.contract, me.symbol, me.sell sell, me.buy buy, 'hotbit' site FROM module_hotbit me
            ...
            bancor as (SELECT tp.id, tp.tsymbol, tp.contract, me.exch_direction, me.highest_bid sell, me.lowest_ask buy, 'bancor' site FROM module_bancor me
             LEFT JOIN trusted_pairs tp ON lower(tp.tsymbol) = lower(me.tsymbol) WHERE me.is_active is true AND tp.is_active is true AND (me.highest_bid is not null or me.lowest_ask is not null) ),
            
            kyber as (SELECT tp.id, tp.tsymbol, tp.contract, me.exch_direction, me.highest_bid sell, me.lowest_ask buy, 'kyber' site FROM module_kyber me
             LEFT JOIN trusted_pairs tp ON lower(tp.tsymbol) = lower(me.tsymbol) WHERE me.is_active is true AND tp.is_active is true AND (me.highest_bid is not null or me.lowest_ask is not null) ),
            
            uniswap as (SELECT tp.id, tp.tsymbol, tp.contract, me.exch_direction, me.highest_bid sell, me.lowest_ask buy, 'uniswap' site FROM module_uniswap me
             LEFT JOIN trusted_pairs tp ON lower(tp.tsymbol) = lower(me.tsymbol) WHERE me.is_active is true AND tp.is_active is true AND (me.highest_bid is not null or me.lowest_ask is not null) ),
            
            uniswap_one as (SELECT tp.id, tp.tsymbol, tp.contract, me.exch_direction, me.highest_bid sell, me.lowest_ask buy, 'uniswap_one' site FROM module_uniswap_one me
             LEFT JOIN trusted_pairs tp ON lower(tp.tsymbol) = lower(me.tsymbol) WHERE me.is_active is true AND tp.is_active is true AND (me.highest_bid is not null or me.lowest_ask is not null) )
