...
async def compare_markets(btoken, all_tokens, percent, currency, cnt, currencyUSD):
    bilaxy_depth = await get_bilaxy_depth(btoken[3], cnt)
    compare_result = []
    if bilaxy_depth:
...
async def init_compare(bilaxy_tokens, all_tokens, percent, currency, currencyUSD):
    ...
    for btoken in bilaxy_tokens:
        async_tasks.append(compare_markets(btoken, all_tokens, percent, currency, cnt, currencyUSD))
        cnt += 1
        if cnt >= len(proxys):
            cnt = 0
...
def bilaxy_profits():
    ...
    all_result = []
    xlen = math.ceil(len(bilaxy_tokens) / 300)
    print(len(bilaxy_tokens))
    for i in range(xlen):
        part_bilaxy_tokens = []
