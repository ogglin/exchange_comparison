...
async def compare(tsymbol, bids, asks, itoken, direction, all_tokens, currency, currencyUSD, percent):
    # print(tsymbol, bids, asks, itoken, direction, currency, currencyUSD, percent)
    compare_result = []
        ...
        if token[0] == tsymbol and 'usd' not in token[0].lower() and 'usd' not in tsymbol.lower():
            ...
            if asks:
                compare_result.append(
                    ct(buy_from='hitbtc', buy_symbol=itoken, buy_prices=asks, buy_volume=0,
            ...
            if bids and ('bancor' in token[2] or 'uniswap' in token[2] or 'kyber' in token[2]
                ...
                compare_result.append(
                    ct(buy_from=token[2], buy_symbol=token[3], buy_prices=token[5], buy_volume=0, sell_to='hitbtc',
...
async def make_compare(tokens, all_tokens, currency, currencyUSD, percent):
    ...
    for key, value in tokens.items():
        if value[0]:
            if len(value[0]['bids']) > 0 or len(value[0]['asks']) > 0:
...
def compare_init():
    ...
    isTD = True
    while isTD:
        if len(gv.replica_hitbtc) > 0 and len(gv.all_compared_tokens) > 0:
            isTD = False
            for token in gv.all_compared_tokens:
                if 'hitbtc' not in token[2]:
    ...
    all_result = loop.run_until_complete(make_compare(gv.replica_hitbtc, all_tokens, currency, currencyUSD, percent))
    loop.close()
    compare_result = rprep(all_result=all_result, exchanger='hitbtc').result()
    print('hitbtc_result', compare_result)
...
async def init_compare_hitbtc():
    while True:
        tstart = datetime.now()
        await compare_init()
        print('hitbtc compare time: ', str(datetime.now() - tstart))
