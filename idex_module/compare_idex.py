        ...
        if token[0] == tsymbol and 'usd' not in token[0].lower() and 'usd' not in tsymbol.lower():
            ...
            if asks:
                compare_result.append(
                    ct(buy_from='idex', buy_symbol=itoken, buy_prices=asks, buy_volume=0,
...
def compare_init():
    ...
    all_result = loop.run_until_complete(make_compare(gv.replica_idex, all_tokens, currency, currencyUSD, percent))
    loop.close()
...
async def init_compare_idex():
    while True:
        tstart = datetime.now()
        await compare_init()
        print('idex compare time: ', str(datetime.now() - tstart))
